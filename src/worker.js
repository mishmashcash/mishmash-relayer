...
async function checkTornadoFee({ args, contract }) {
  ...
  const ethPrice = 1000000000000000000 //await redis.hget('prices', currency)
...
function getProxyContract() {
  const contract = new web3.eth.Contract(tornadoProxyABI, proxyRouter)

  return {
    contract,
    isOldProxy: checkOldProxy(proxyRouter),
  }
}
...
function getTxObject({ data }) {
  if (data.type === jobType.TORNADO_WITHDRAW) {
    let { contract, isOldProxy } = getProxyContract()
